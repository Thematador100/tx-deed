...
import { retrySupabaseQuery, retryWithBackoff } from '@/lib/utils';
  ...
  const updateProfile = useCallback(async (profileData) => {
    ...
    try {
      const { error } = await retrySupabaseQuery(
        ...
        {
          onRetry: ({ attempt }) => {
            console.log(`Retrying profile update (attempt ${attempt})...`);
          }
        }
      );
      ...
      console.error('Failed to update profile after retries:', error);
      return { error };
    }
