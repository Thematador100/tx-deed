"use strict";
// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.
Object.defineProperty(exports, "__esModule", { value: true });
exports.Fqdns = void 0;
const resource_1 = require("../core/resource.js");
const path_1 = require("../internal/utils/path.js");
class Fqdns extends resource_1.APIResource {
    /**
     * Create a new FQDN object.
     *
     * @example
     * ```ts
     * const fqdn = await client.fqdns.create({
     *   connection_id: '1516447646313612565',
     *   dns_record_type: 'a',
     *   fqdn: 'example.com',
     * });
     * ```
     */
    create(body, options) {
        return this._client.post('/fqdns', { body, ...options });
    }
    /**
     * Return the details regarding a specific FQDN.
     *
     * @example
     * ```ts
     * const fqdn = await client.fqdns.retrieve('id');
     * ```
     */
    retrieve(id, options) {
        return this._client.get((0, path_1.path) `/fqdns/${id}`, options);
    }
    /**
     * Update the details of a specific FQDN.
     *
     * @example
     * ```ts
     * const fqdn = await client.fqdns.update('id');
     * ```
     */
    update(id, body = {}, options) {
        return this._client.patch((0, path_1.path) `/fqdns/${id}`, { body, ...options });
    }
    /**
     * Get all FQDNs belonging to the user that match the given filters.
     *
     * @example
     * ```ts
     * const fqdns = await client.fqdns.list();
     * ```
     */
    list(query = {}, options) {
        return this._client.get('/fqdns', { query, ...options });
    }
    /**
     * Delete an FQDN.
     *
     * @example
     * ```ts
     * const fqdn = await client.fqdns.delete('id');
     * ```
     */
    delete(id, options) {
        return this._client.delete((0, path_1.path) `/fqdns/${id}`, options);
    }
}
exports.Fqdns = Fqdns;
//# sourceMappingURL=fqdns.js.map