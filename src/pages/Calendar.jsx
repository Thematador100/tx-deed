  ...
  useEffect(() => {
    const fetchCalendarEvents = async () => {
      ...
      const { data: redemptionData, error: redemptionError } = await supabase
        ...
        .not('redemption_date', 'is', null);

      const allEvents = [];

      if (!auctionError && auctionData) {
        const taxSaleEvents = auctionData.map(p => ({
          id: p.id,
          title: `Auction: ${p.address}`,
          date: parseISO(p.auction_date),
          type: 'auction',
          link: `/property/${p.id}`
        }));
        allEvents.push(...taxSaleEvents);
      }

      if (!redemptionError && redemptionData) {
        const redeemableEvents = redemptionData.map(d => ({
          id: d.id,
        ...
        allEvents.push(...redeemableEvents);
      }
