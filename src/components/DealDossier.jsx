...
const DealDossier = ({ property }) => {
  ...
  const handleGenerateDossier = async () => {
    ...
    try {
      const { data, error: functionError } = await supabase.functions.invoke('generate-dossier', {
  ...
  const handleOcrUpload = async () => {
    ...
    try {
      ...
      // 2. Invoke the OCR function
      const { data, error: functionError } = await supabase.functions.invoke('analyze-document-ocr', {
        body: { filePath },
      });
