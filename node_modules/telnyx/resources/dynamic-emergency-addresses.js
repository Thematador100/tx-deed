"use strict";
// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.
Object.defineProperty(exports, "__esModule", { value: true });
exports.DynamicEmergencyAddresses = void 0;
const resource_1 = require("../core/resource.js");
const path_1 = require("../internal/utils/path.js");
class DynamicEmergencyAddresses extends resource_1.APIResource {
    /**
     * Creates a dynamic emergency address.
     *
     * @example
     * ```ts
     * const dynamicEmergencyAddress =
     *   await client.dynamicEmergencyAddresses.create({
     *     administrative_area: 'TX',
     *     country_code: 'US',
     *     house_number: 'house_number',
     *     locality: 'Austin',
     *     postal_code: '78701',
     *     street_name: 'Congress',
     *   });
     * ```
     */
    create(body, options) {
        return this._client.post('/dynamic_emergency_addresses', { body, ...options });
    }
    /**
     * Returns the dynamic emergency address based on the ID provided
     *
     * @example
     * ```ts
     * const dynamicEmergencyAddress =
     *   await client.dynamicEmergencyAddresses.retrieve(
     *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
     *   );
     * ```
     */
    retrieve(id, options) {
        return this._client.get((0, path_1.path) `/dynamic_emergency_addresses/${id}`, options);
    }
    /**
     * Returns the dynamic emergency addresses according to filters
     *
     * @example
     * ```ts
     * const dynamicEmergencyAddresses =
     *   await client.dynamicEmergencyAddresses.list();
     * ```
     */
    list(query = {}, options) {
        return this._client.get('/dynamic_emergency_addresses', { query, ...options });
    }
    /**
     * Deletes the dynamic emergency address based on the ID provided
     *
     * @example
     * ```ts
     * const dynamicEmergencyAddress =
     *   await client.dynamicEmergencyAddresses.delete(
     *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
     *   );
     * ```
     */
    delete(id, options) {
        return this._client.delete((0, path_1.path) `/dynamic_emergency_addresses/${id}`, options);
    }
}
exports.DynamicEmergencyAddresses = DynamicEmergencyAddresses;
//# sourceMappingURL=dynamic-emergency-addresses.js.map