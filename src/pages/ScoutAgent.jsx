...
import { UserCheck, PlusCircle, Loader2, Trash2, Edit, Bell, Mail, MessageSquare, Eye } from 'lucide-react';
...
const ScoutAgentForm = ({ agent, onSave, onCancel }) => {
  ...
  const [selectedCountyIds, setSelectedCountyIds] = useState(agent?.criteria?.county_ids || []);
  ...
  const handleSave = () => {
    ...
    if (selectedCountyIds.length === 0) {
      toast({ title: "Please select at least one county", variant: "destructive" });
      return;
    }
    const criteria = {
      county_ids: selectedCountyIds,
      propertyType,
      minBeds,
      minBaths,
      minScore,
  ...
  return (
    <div className="grid gap-6 py-4">
      ...
      <div className="grid grid-cols-4 items-start gap-4">
        <Label className="text-right mt-2">Target Counties</Label>
        <div className="col-span-3">
          <CountySelector
            selectedCounties={selectedCountyIds}
            onChange={setSelectedCountyIds}
            maxSelections={50}
          />
          <p className="text-xs text-slate-500 mt-2">
            Select up to 50 counties to monitor for tax deed opportunities
          </p>
        </div>
      </div>
...
const AgentCard = ({ agent, onToggleActive, onEdit, onDelete, onShowFinds }) => {
  ...
  // Fetch county names for display
  useEffect(() => {
    const fetchCountyNames = async () => {
      if (agent.criteria.county_ids && agent.criteria.county_ids.length > 0) {
        const { data } = await supabase
          .from('counties')
          .select('county_name, state_code')
          .in('id', agent.criteria.county_ids)
          .limit(5);
    ...
    fetchCountyNames();
  }, [agent.criteria.county_ids]);

  const fetchFinds = useCallback(async () => {
    setLoadingFinds(true);
    let query = supabase
      .from('properties')
      .select('id, address, opportunity_score, county_id')
    ...
    if (agent.criteria.county_ids && agent.criteria.county_ids.length > 0) {
      query = query.in('county_id', agent.criteria.county_ids);
    }
  ...
  }, [agent.criteria.minScore, agent.criteria.county_ids]);
    ...
    >
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">
              Monitoring <span className="font-semibold">
                {countyNames.length > 0
                  ? `${countyNames.map(c => `${c.county_name}, ${c.state_code}`).join('; ')}${
                      agent.criteria.county_ids?.length > 5 ? ` +${agent.criteria.county_ids.length - 5} more` : ''
                    }`
                  : `${agent.criteria.county_ids?.length || 0} counties`}
              </span> with score <span className="font-semibold">&gt;{agent.criteria.minScore}</span>.
            </p>
          </div>
      ...
      <div className="p-4 bg-white flex justify-between items-center">
        <p className="text-xs text-slate-500 flex items-center">
          <Bell className="w-3 h-3 mr-1.5" /> Notifying via {agent.notification_method}.
        </p>
...
const ScoutAgent = () => {
  ...
  const handleShowFinds = (agent) => {
    ...
    if (agent.criteria.county_ids && agent.criteria.county_ids.length > 0) {
      searchParams.set('county_ids', agent.criteria.county_ids.join(','));
    }
