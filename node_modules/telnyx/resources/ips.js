"use strict";
// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.
Object.defineProperty(exports, "__esModule", { value: true });
exports.IPs = void 0;
const resource_1 = require("../core/resource.js");
const path_1 = require("../internal/utils/path.js");
class IPs extends resource_1.APIResource {
    /**
     * Create a new IP object.
     *
     * @example
     * ```ts
     * const ip = await client.ips.create({
     *   ip_address: '192.168.0.0',
     * });
     * ```
     */
    create(body, options) {
        return this._client.post('/ips', { body, ...options });
    }
    /**
     * Return the details regarding a specific IP.
     *
     * @example
     * ```ts
     * const ip = await client.ips.retrieve(
     *   '6a09cdc3-8948-47f0-aa62-74ac943d6c58',
     * );
     * ```
     */
    retrieve(id, options) {
        return this._client.get((0, path_1.path) `/ips/${id}`, options);
    }
    /**
     * Update the details of a specific IP.
     *
     * @example
     * ```ts
     * const ip = await client.ips.update(
     *   '6a09cdc3-8948-47f0-aa62-74ac943d6c58',
     *   { ip_address: '192.168.0.0' },
     * );
     * ```
     */
    update(id, body, options) {
        return this._client.patch((0, path_1.path) `/ips/${id}`, { body, ...options });
    }
    /**
     * Get all IPs belonging to the user that match the given filters.
     *
     * @example
     * ```ts
     * const ips = await client.ips.list();
     * ```
     */
    list(query = {}, options) {
        return this._client.get('/ips', { query, ...options });
    }
    /**
     * Delete an IP.
     *
     * @example
     * ```ts
     * const ip = await client.ips.delete(
     *   '6a09cdc3-8948-47f0-aa62-74ac943d6c58',
     * );
     * ```
     */
    delete(id, options) {
        return this._client.delete((0, path_1.path) `/ips/${id}`, options);
    }
}
exports.IPs = IPs;
//# sourceMappingURL=ips.js.map