...
const MemberDashboard = () => {
  ...
  const handleTestNotification = async () => {
    ...
    const notificationTypes = [
      {
        ...
        message: 'A new property matching your criteria has been added: 123 Main St, Austin, TX',
        link: '/properties'
      },
      {
        type: 'auction',
        title: '⚖️ Auction Starting Soon',
        message: 'Auction for property at 456 Oak Ave starts in 1 hour!',
        link: '/auctions-leads'
      },
      {
        type: 'deal',
        title: '💼 Deal Update',
        message: 'Your deal pipeline has been updated with new opportunities',
        link: '/my-pipeline'
      }
    ];
    ...
    const result = await notificationService.sendNotification(user.id, randomNotification);
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <button onClick={handleTestNotification} className="flex flex-col items-center text-center space-y-2 p-4 bg-slate-50 hover:bg-green-100 rounded-xl transition-colors border border-slate-200 hover:border-green-200 w-full">
                  <div className="w-10 h-10 flex items-center justify-center bg-green-100 rounded-lg"><Bell className="w-5 h-5 text-green-600" /></div>
                  <span className="text-slate-800 font-semibold text-sm">Test Notification</span>
                </button>
              </div>
            </motion.div>
