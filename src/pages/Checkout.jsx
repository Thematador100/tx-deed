...
import { retrySupabaseQuery, retryEdgeFunction } from '@/lib/utils';
...
const Checkout = () => {
  ...
  const handleCheckout = async () => {
    ...
    // Create a transaction/invoice record in your database with retry
    try {
      const { data: transaction, error: transactionError } = await retrySupabaseQuery(
        () => supabase
          .from('transactions')
          .insert({
            user_id: user.id,
        ...
        {
          onRetry: ({ attempt }) => {
            console.log(`Retrying transaction creation (attempt ${attempt})...`);
          }
        }
      );
      ...
      // Create a Stripe Checkout session via a Supabase Edge Function with retry
      const { data, error } = await retryEdgeFunction(
        ...
        {
          onRetry: ({ attempt }) => {
            console.log(`Retrying checkout session creation (attempt ${attempt})...`);
          }
        }
      );
      ...
      toast({ title: "Checkout Error", description: "Payment processing failed. Please try again later.", variant: "destructive" });
      setLoading(false);
      return;
    }
