  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (stagesError || !stagesData || stagesData.length === 0) {
        toast({
          title: "Error Loading Pipeline Stages",
          description: "Could not fetch pipeline stages. Please ensure your database is set up correctly.",
          variant: "destructive"
        });
        // Set default stages structure
        const defaultStages = [
        ...
        if (user && !isDemo) {
          ...
          if (propsError) {
            console.error('Error fetching saved properties:', propsError);
            setProperties([]);
          } else {
