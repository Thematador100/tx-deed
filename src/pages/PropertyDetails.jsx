...
import { retrySupabaseQuery } from '@/lib/utils';
...
const PropertyDetails = () => {
    ...
    const fetchProperty = async () => {
      ...
      try {
        const { data, error } = await retrySupabaseQuery(
          ...
          {
            onRetry: ({ attempt }) => {
              console.log(`Retrying property fetch (attempt ${attempt})...`);
            }
          }
        );
        ...
        console.error('Failed to fetch property after retries:', error);
        toast({
          title: "Error",
  ...
  const handleSaveToPipeline = async () => {
    ...
    try {
      ...
      const { data: initialStage } = await retrySupabaseQuery(
        () => supabase.from('pipeline_stages').select('id').order('sort_order').limit(1).single(),
        {
          onRetry: ({ attempt }) => {
            console.log(`Retrying pipeline stage fetch (attempt ${attempt})...`);
          }
        }
      );
      if (initialStage) {
      ...
      const { data: existing, error: checkError } = await retrySupabaseQuery(
        () => supabase
        ...
        {
          onRetry: ({ attempt }) => {
            console.log(`Retrying saved property check (attempt ${attempt})...`);
          }
        }
      ).catch(err => {
        // PGRST116 means no rows returned, which is expected for new saves
      ...
      const { error } = await retrySupabaseQuery(
        ...
        {
          onRetry: ({ attempt }) => {
            console.log(`Retrying property save (attempt ${attempt})...`);
          }
        }
      );
      ...
      console.error('Failed to save property to pipeline after retries:', error);
      toast({ title: "Error", description: "Failed to save property. Please try again later.", variant: "destructive" });
    }
    setIsSaving(false);
  };
